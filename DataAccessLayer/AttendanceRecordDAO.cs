...
namespace DataAccessLayer
{
    public class AttendanceRecordDAO
    {
        ...
        public async Task<List<AttendanceRecord>> GetByDateRangeAsync(DateTime startDate, DateTime endDate)
        {
            return await _context.AttendanceRecords
                .Include(ar => ar.User)
                .Where(ar => ar.RecordTime >= startDate && ar.RecordTime <= endDate)
                .ToListAsync();
        }

        public async Task<AttendanceRecord?> GetByUserIdAndDateAsync(int userId, DateTime date, string type)
        {
            return await _context.AttendanceRecords
                .FirstOrDefaultAsync(ar => ar.UserId == userId
                                        && ar.RecordTime.Date == date.Date
                                        && ar.Type == type);
        }
    }
}
